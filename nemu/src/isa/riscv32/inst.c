...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("000000 ?????? ????? 001 ????? 00100 11", slli   , I, R(rd) = src1 << (imm % WORD_LEN));
  INSTPAT("000000 ?????? ????? 101 ????? 00100 11", srli   , I, R(rd) = src1 >> (imm % WORD_LEN));
  INSTPAT("010000 ?????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (sword_t)src1 >> (imm % WORD_LEN));
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw   , R, R(rd) = SEXT((uint32_t)(src1 << (src2 % 32)), 32));
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srlw   , R, R(rd) = SEXT((uint32_t)src1 >> (src2 % 32), 32));
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, R(rd) = SEXT((int32_t)src1 >> (src2 % 32), 32));

  INSTPAT("0000001 ????? ????? 000 ????? 01110 11", mulw   , R, R(rd) = (word_t)(SEXT((uint32_t)(src1 * src2), 32)));
